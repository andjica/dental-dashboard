...
const subCategorytId = route.params.id;

// Form values
const subCategoryName = ref("");
const selectedCategory = ref("");
const originalSubCategoryName = ref("");
...
const fetchSubCategory = async () => {
  try {
    const response = await get(`subcategories/${subCategorytId}`);
    const data = response.subcategory || response.data || response;

    subCategoryName.value = data.name;
    selectedCategory.value = data.category_id;

    originalSubCategoryName.value = data.name;
    originalCategoryId.value = data.category_id;
...
const handleSubmit = async () => {
  ...
  if (
    subCategoryName.value.trim() === originalSubCategoryName.value.trim() &&
    selectedCategory.value === originalCategoryId.value
    ) {
    showAlert.value = true;
  ...
  try {
    await put(`subcategories/${subCategorytId}`, {
